...
MODEL_CACHE = "stable-diffusion-3.5-large"
MODEL_URL_DEV = "https://weights.replicate.delivery/default/stabilityai/stable-diffusion-3.5-large/full.tar"
...
class Predictor(BasePredictor):
    def setup(self) -> None:
        ...
        if not os.path.exists(MODEL_CACHE):
            download_weights(MODEL_URL_DEV, MODEL_CACHE)

    # LoRA + DreamBooth
    def predict(
        self,
        input_images: Path = Input(description="A zip file containing the images that will be used for training.", default=None,),
        instance_prompt: str = Input(description="Instance prompt to trigger the image generation", default="Frog, yarn art style"),
        resolution: int = Input(description="The resolution for input images, all the images in the train/validation dataset will be resized to this", default=768, choices=[512,768,1024]),
        max_train_steps: int = Input(description="Total number of training steps to perform", default=700, ge=100, le=6000),
        rank: int = Input(description="The dimension of the LoRA", default=16, ge=4, le=64),
        ...
        hub_model_id: str = Input(description="Huggingface model location for upload. Requires a HF token with write permissions. Ex: lucataco/SD3.5-Large-queso", default=None),
        ...
        output_dir = output_base + "/output/sd35_large_train_replicate"
